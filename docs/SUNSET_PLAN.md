...
- 配布物 (`target/release/bundle/`)
  - [ ] 直近ビルド済みの AppImage/MSI/DMG をウイルススキャン・署名検証済みのストレージへ退避し、取得元コミットハッシュを添えて目録化する。
...
- Ollama サーバーが 0.2x 系へ更新され、`/api/generate` のストリーミングレスポンス形式が確定。`run_ollama_stream` の互換レイヤーを Rust 側で検証済みであること。
